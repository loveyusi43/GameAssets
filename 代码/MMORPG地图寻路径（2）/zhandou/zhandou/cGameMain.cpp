...
cGameMain::cGameMain(void)
{
	...
	m_seeSencePt.x = 0;
	m_seeSencePt.y = 0;
	m_pathManager = new cStarPath(L"bbb.map");
}
...
void cGameMain::Init(HWND hWnd)
{
	...
	for (int i =1;i<=8;i++)
	{
		wsprintf(szFilePath,L"temp\\bing_%02d.bmp",i);
		pathlist.push_back(szFilePath);
	}
	m_pRole = new cRoleObject;
	m_pRole->m_roleStatus = Status_Stop;

	m_pRole->m_pPictureInfo = m_pSence->AddAnimation(pathlist,pt,Layer_Main_Player,RGB(34,177,76));
...
void cGameMain::MoveMap()
{
	...
	m_pSence->m_ptScreenToMap = m_seeSencePt;
...
void cGameMain::RoleMove(LPARAM lParam)
{
	//角色移动
	tagPOINT pt;//点击的点
	...
	//获得点击处相对于大地图的坐标
	pt.x = m_seeSencePt.x+pt.x;
	pt.y = m_seeSencePt.y+pt.y;
	m_pathManager->MoveTo(m_pRole,pt);
}